<div class="widget-content widget-content-area icon-pill" id="gmz-settings-page">
    ...
    $_item['tabs'] = BaseGateway::inst()->getPaymentSettings();
                                ...
                                <ul class="nav nav-tabs  mb-3 mt-3" id="{{$_item['id']}}SettingsTab" role="tablist">
                                    ...
                                    @if($_item['id']=="service_tab")
                                        @foreach($_item['tabs'] as $val_tab)
                                            @if($val_tab['id']=="hotel_service" || $val_tab['id']=="apartment_service" || $val_tab['id']=="tour_service" || $val_tab['id']=="space_service")

                                                <li class="nav-item">
                                                    <a class="nav-link @if($loop->index == 0) active @endif"
                                                       id="{{$val_tab['id']}}-tab" data-toggle="tab" href="#{{$val_tab['id']}}"
                                                       role="tab" aria-controls="{{$val_tab['id']}}" aria-selected="true">
                                                        @if($val_tab['heading']=="Apartment")
                                                            <span>Appartement</span>
                                                        @elseif($val_tab['heading']=="Space")
                                                            <span>Villas</span>
                                                        @elseif($val_tab['heading']=="Tour")
                                                            <span>Salles évenementielles</span>
                                                        @else
                                                            {{$val_tab['heading']}}
                                                        @endif
