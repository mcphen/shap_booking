                ...
                @if($allUsers->total() > 0)
                    <table class="multi-table table table-striped table-bordered table-hover non-hover w-100" data-plugin="footable">
                        ...
                        <tr>
                            ...
                            <th data-breakpoints="xs sm">{{__('Piece identite')}}</th>
                            <th data-breakpoints="xs sm">{{__('Justificatif')}}</th>
                        ...
                        @foreach($allUsers->items() as $key => $item)
                            <tr>
                                ...
                                <td>
                                    <a href="{{\Illuminate\Support\Facades\Storage::url($item->piece_identite_path)}}"><i class="fa fa-file"></i></a>
                                </td>
                                <td>
                                    <a href="{{\Illuminate\Support\Facades\Storage::url($item->justificatif_path)}}"><i class="fa fa-file"></i></a>
                                </td>
                                <td>
                                    @php
                                        $approve_params = [
